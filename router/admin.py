...
def get_issue_books(
    ...
    issues = db.query(IssueRecords).order_by(
        IssueRecords.id.desc()
    ).all()
    ...
    for issue in issues:
        ...
        result.append({
            ...
            "fine_amount": (
                issue.fine_amount
                if issue.fine_amount is not None
                else 0
            ),

            "fine_paid": (
                issue.fine_paid
                if issue.fine_paid is not None
                else False
            )
        })
